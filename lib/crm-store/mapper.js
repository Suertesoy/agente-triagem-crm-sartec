...
export function mapRedisContact(phoneValue, contact = {}, { includeLegacyContact = true } = {}) {
  ...
  return {
    customer: {
      ...
      ...(includeLegacyContact ? { legacy_contact: structuredClone(contact) } : {}),
...
export function mapLiveCustomer(phoneValue, contact = {}) {
  return mapRedisContact(phoneValue, contact, { includeLegacyContact: false });
}
...
export function mapRedisConversation(
  phoneValue,
  session = {},
  { includeLegacySession = false, firstMessageAt = null } = {}
) {
  ...
  if (includeLegacySession) {
    ...
    legacyFields.legacy_session = legacySession;
  }

  return {
    conversation: {
      id: deterministicUuid("sartec-conversation", redisKey),
...
export function mapLiveConversation(phoneValue, session = {}) {
  return mapRedisConversation(phoneValue, session, { includeLegacySession: false });
}
...
export function mapRedisSession(phoneValue, session = {}) {
  ...
  const firstMessageAt = messages
    ...
    .filter(Boolean)
    .sort()[0] || null;
  const { conversation, unmappedFields } = mapRedisConversation(phone, session, {
    includeLegacySession: true,
    firstMessageAt,
  });
