...
As quatro primeiras migrations representam o estado remoto existente. A migration incremental foi registrada remotamente como `20260817175639_add_legacy_message_audit_fields` e o arquivo local usa o mesmo timestamp.

`20260818151936_20260817231131_ordered_crm_shadow_write.sql` prepara revisões condicionais, RPCs server-only e `crm_settings`. Ela foi aplicada remotamente na versão `20260818151936`, com o nome `20260817231131_ordered_crm_shadow_write`. A infraestrutura correspondente não está conectada aos endpoints; Redis continua como fonte operacional e `SUPABASE_DUAL_WRITE=false` é o padrão.
